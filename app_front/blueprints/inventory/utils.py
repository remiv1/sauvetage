...
def validate_inventory(
    lines: List[Dict[str, Any]], inventory_type: str = "partial"
) -> Dict[str, Any]:
...
def commit_inventory(
    planned: List[Dict[str, Any]], inventory_type: str = "partial"
) -> Dict[str, Any]:
...
def create_product(product_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if product_type not in ("book", "other"):
        raise ValueError(
            f"product_type invalide : '{product_type}'. "
    ...
    try:
        # EAN13 déjà existant ?
        existing = (
            session.execute(select(GeneralObjects).where(GeneralObjects.ean13 == ean13))
            .scalars()
            .first()
        )
        if existing:
        ...
        # Fournisseur valide ?
        supplier = (
            session.execute(select(Suppliers).where(Suppliers.id == supplier_id))
            .scalars()
            .first()
        )
        if not supplier:
...
def search_objects_info(q: dict[str, str]) -> List[str]:
    ...
    if modal[0] == "author":
        ...
        stmt = (
            select(Books.author)
            .where(
                func.unaccent(func.lower(Books.author)).ilike(
                    func.unaccent(func.lower(f"%{req}%"))
                )
            )
            .distinct()
        )
        ...
        req = q["diffuser"]
        stmt = (
            select(Books.diffuser)
            .where(
                func.unaccent(func.lower(Books.diffuser)).ilike(
                    func.unaccent(func.lower(f"%{req}%"))
                )
            )
            .distinct()
        ...
        stmt = (
            select(Books.editor)
            .where(
                func.unaccent(func.lower(Books.editor)).ilike(
                    func.unaccent(func.lower(f"%{req}%"))
                )
            )
            .distinct()
        ...
        stmt = (
            select(Books.genre)
            .where(
                func.unaccent(func.lower(Books.genre)).ilike(
                    func.unaccent(func.lower(f"%{req}%"))
                )
            )
            .distinct()
