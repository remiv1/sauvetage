...
class EanInputForm(FlaskForm):
    ...
    ean_textarea = TextAreaField("Liste d'EAN13", validators=[Optional()])
    ean_single = StringField(
        "Code EAN13", validators=[Optional(), Length(min=13, max=13)]
    )
...
class ProductCreateForm(FlaskForm):
    ...
    general_object_type = SelectField(
        "Type de produit",
        choices=[("book", "Livre"), ("other", "Autre (jeu, objet de piété…)")],
        default="book",
        validators=[DataRequired()],
    )
    add_to_dilicom = BooleanField("Ajouter à la base Dilicom", default=False)
    supplier_id = HiddenField("Fournisseur (ID)", validators=[DataRequired()])
    supplier_name = StringField(
        "Fournisseur",
        validators=[DataRequired(), Length(max=255)],
        render_kw={"autocomplete": "off"},
    )
    name = StringField("Titre / Nom", validators=[DataRequired(), Length(max=255)])
    description = TextAreaField("Description", validators=[Optional()])
    price = DecimalField(
        "Prix", places=2, validators=[DataRequired(), NumberRange(min=0)]
    )
    ...
    diffuser = StringField("Distributeur", validators=[Optional(), Length(max=255)])
    editor = StringField("Éditeur", validators=[Optional(), Length(max=255)])
    genre = StringField("Catégorie/Genre", validators=[Optional(), Length(max=120)])
    publication_year = StringField(
        "Année de publication", validators=[Optional(), Length(max=4)]
    )
    pages = StringField("Nombre de pages", validators=[Optional(), Length(max=10)])
...
class TagCreateForm(FlaskForm):
    ...
    name = StringField("Nom du tag", validators=[DataRequired(), Length(max=50)])
    description = TextAreaField(
        "Description du tag", validators=[Optional(), Length(max=255)]
    )
...
class JoinTagObjectForm(FlaskForm):
    ...
    tag_id = SelectField(
        "Tag", validators=[DataRequired()]  # Rempli dynamiquement en JS
    )
...
class MetadataForm(FlaskForm):
    ...
    general_object_id = HiddenField(GENERAL_OBJECT_STRING, validators=[DataRequired()])
    key = StringField("Clé", validators=[DataRequired(), Length(max=50)])
    value = StringField("Valeur", validators=[DataRequired(), Length(max=255)])
...
class MediaFilesForm(FlaskForm):
    ...
    file_data = FileField("Fichier", validators=[DataRequired()])
    file_link = StringField("URL du fichier", validators=[Optional(), Length(max=255)])
    description = StringField(
        "Description du fichier", validators=[Optional(), Length(max=255)]
    )
...
class SupplierCreateForm(FlaskForm):
    ...
    name = StringField(
        "Nom du fournisseur", validators=[Optional(), Length(min=1, max=255)]
    )  # Optional pour éviter required="" en HTML
    gln13 = StringField("GLN13 (optionnel)", validators=[Optional(), Length(max=13)])
    contact_email = StringField("Email", validators=[Optional(), Email()])
    contact_phone = StringField("Téléphone", validators=[Optional(), Length(max=20)])
