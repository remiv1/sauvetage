...
def _add_a_year(start: datetime) -> datetime:
    ...
    try:
        return datetime(
                ...
                day=int(start.day)) - timedelta(days=1)
    except ValueError:
        # Cas du 29 février
        return datetime(
                year=int(start.year) + 1,
...
def _add_a_semester(start: datetime) -> datetime:
    ...
    year = int(start.year)
    if month <= 6:
        return datetime(year=year, month=month + 6, day=int(start.day)) - timedelta(days=1)
    return datetime(year=year + 1, month=month - 6, day=int(start.day)) - timedelta(days=1)
...
def _add_a_trimestre(start: datetime) -> datetime:
    ...
    year = int(start.year)
    if month <= 9:
        return datetime(year=year, month=month + 3, day=int(start.day)) - timedelta(days=1)
    return datetime(year=year + 1, month=month - 9, day=int(start.day)) - timedelta(days=1)
