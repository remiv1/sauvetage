...
class OrderIn(WorkingBase, QueryMixin):
    ...
    def __repr__(self) -> str:
        return (
            f"<OrderIn(id={self.id}, external_ref={self.external_ref}, "
...
class OrderInLine(WorkingBase, QueryMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    order_in_id: Mapped[int] = mapped_column(
        Integer,
...
class DilicomReferencial(WorkingBase, QueryMixin):
    ...
    ean13: Mapped[str] = mapped_column(
        String,
        ForeignKey("app_schema.general_objects.ean13"),
        nullable=False,
    ...
    create_ref: Mapped[bool] = mapped_column(
        Boolean, default=False, comment="Indique si l'objet doit être créé"
    )
    delete_ref: Mapped[bool] = mapped_column(
        Boolean, default=False, comment="Indique si l'objet doit être supprimé"
    )
    is_active: Mapped[bool] = mapped_column(
        Boolean, default=True, comment="Indique si la référence est active"
    )
    dilicom_synced: Mapped[bool] = mapped_column(
        Boolean, default=False, comment="Synchronisé avec Dilicom ou non"
    )
    ...
    general_object = relationship(
        "GeneralObjects", back_populates="dilicom_referencial"
    )
    supplier = relationship("Suppliers", back_populates="dilicom_referencial")

    def __repr__(self) -> str:
        return (
            f"<DilicomReferencial(id={self.id}, ean13={self.ean13}, gln13={self.gln13}, "
