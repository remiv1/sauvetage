...
if [ "${LETSENCRYPT:-false}" = "true" ] || [ "${LETSENCRYPT}" = "1" ]; then
    echo "[ENTRYPOINT] LETSENCRYPT enabled — preparing ACME storage..."
    ACME_FILE="$PKI_DIR/acme.json"
    if [ ! -f "$ACME_FILE" ]; then
        echo "{}" > "$ACME_FILE"
        chmod 600 "$ACME_FILE"
        echo "[ENTRYPOINT] Created $ACME_FILE"
    else
        chmod 600 "$ACME_FILE"
        echo "[ENTRYPOINT] ACME storage present"
    fi
    echo "[ENTRYPOINT] Skipping internal PKI generation (LETSENCRYPT active)"
else
